# Git commit and push

Commit all changes with a short 1-2 sentence message and push to the remote.

## Steps

1. Run `git status` to see what has changed.
2. Run `git diff` (and `git diff --staged` if needed) to understand the changes.
3. Write a short commit message (1-2 sentences) that summarizes the changes. Be specific and descriptive.
4. Run `git add -A` to stage all changes.
5. Run `git commit -m "Your message here"` with the message you wrote.
6. Run `git push` to push to the remote.

If the user provides a commit message when invoking this command, use that instead of generating one. Otherwise, generate the message from the diff.

Do not use `--no-verify` or skip hooks unless the user explicitly asks. If the push fails, report the error clearly.
