...
The extension does **not** reimplement the MCP server. It spawns `pinrag-mcp` (or `uvx pinrag` / `python -m pinrag.cli`) and connects via stdio.
...
| **pipx** | `pipx run pinrag` | Common for Python tools | Requires pipx |
...
**Recommended:** Use `uvx pinrag` (or `uv tool run pinrag`) as the default. Document that `uv` must be installed. Fallback: check for `pinrag-mcp` on PATH and use that if present.
...
2. **Implement:** Add `mcpServerDefinitionProviders` and spawn `uvx pinrag` (or `pinrag-mcp`)
...
Requires [uv](https://docs.astral.sh/uv/) on your PATH. Alternatively, install PinRAG first (`pipx install pinrag`) and use [this config](vscode:mcp/install?...) with `command: "pinrag-mcp"`.
```
