...
def test_get_use_parent_child_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("PINRAG_USE_PARENT_CHILD", raising=False)
...
def test_get_use_parent_child_true_values(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for val in ("true", "1", "yes", "on"):
        monkeypatch.setenv("PINRAG_USE_PARENT_CHILD", val)
...
def test_create_retriever_returns_chroma_when_parent_child_off(
    ...
    """When PINRAG_USE_PARENT_CHILD=false, create_retriever returns Chroma retriever."""
    monkeypatch.setenv("PINRAG_USE_PARENT_CHILD", "false")
    persist = tmp_path / "chroma_test"
    persist.mkdir(parents=True)
    with patch("pinrag.vectorstore.retriever.get_chroma_store") as mock:
