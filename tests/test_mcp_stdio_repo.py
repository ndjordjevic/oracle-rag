...
Verbose progress: run pytest with ``--log-cli-level=INFO``, e.g.::
    ...
    pytest tests/test_mcp_stdio_repo.py -v -m integration --log-cli-level=INFO
"""
...
from tests.conftest import require_working_llm_for_default_provider
...
def test_pdf_roundtrip_local_repo(tmp_path: Path) -> None:
    ...
    require_working_llm_for_default_provider("MCP stdio local repo")
    uv_bin = _uv_bin()
    if not uv_bin:
